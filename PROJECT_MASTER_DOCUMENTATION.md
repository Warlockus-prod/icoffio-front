...
**Версия:** v7.28.0 (Admin Panel Overhaul)  
**Дата:** 2025-12-04  
**Статус:** ✅ PRODUCTION READY

## 🔄 ПОСЛЕДНИЕ ОБНОВЛЕНИЯ v7.28.0
...
1. Парсится русский URL → определяется язык (RU)
2. **Автоматический перевод на EN** (становится основной версией)
3. **Автоматический перевод на PL** (вторичная версия)
4. В Editor показывается **АНГЛИЙСКАЯ версия** (не русская!)
5. Русская версия нигде не сохраняется и не показывается
6. Админ работает с EN версией, может переключиться на PL

**Файл:** `lib/stores/admin-store.ts` строки 538-542
```typescript
// ВСЕГДА берем английскую версию как основную
const enPost = posts.en || posts[Object.keys(posts)[0]];
article.title = enPost.title;     // ✅ EN как основной!
article.content = enPost.content; // ✅ EN как основной!
```
