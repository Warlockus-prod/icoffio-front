...
### 🎯 Что сделано
- TODO/FIXME/HACK из production-кода вынесены в приоритизированный tech-debt backlog.
- Расширен manifest синхронизации на дополнительные критичные API и admin UI пути.
- Добавлен формальный план этапа 2 консолидации структуры.

### 🔧 Реализация
- `docs/TECH_DEBT_BACKLOG.md`
- `docs/CONSOLIDATION_STAGE2_PLAN.md`
- `docs/SOURCE_OF_TRUTH_WORKFLOW.md` (обновлен scope и ссылки на backlog/plan)
- `sync-manifest.json` (увеличено покрытие mirror guard)
...
### 🎯 Что зафиксировано
- Исправлена публикация из админки: выбранное изображение теперь стабильно становится `hero`, а не дефолтный placeholder.
- Исправлен `AI Generate` в URL Parser: текстовые AI-job больше не должны падать в `failed` из-за тяжёлого пайплайна.
- В `All Articles` удалены случайные просмотры и добавлен fallback миниатюр при недоступных URL.
- Добавлена дополнительная зачистка markdown-артефактов в summary/excerpt для EN/PL.

### 🔧 Изменения в коде
- `components/admin/ArticleCreatorModal.tsx`
  - Нормализация выбора hero-изображения и очередности изображений.
  - Корректная отправка `image/images` в публикацию.
- `app/api/articles/route.ts`
  - Нормализация входных изображений перед сохранением.
  - Поддержка `stage`, `enhanceContent`, `generateImage`, `translateToAll` для text/url потоков.
- `components/admin/URLParser/AIGenerate.tsx`
  - Генерация контента через `/api/admin/generate-article-content` + fallback.
  - Отправка в очередь с облегченными настройками text-pipeline.
- `components/admin/URLParser/ParsingQueue.tsx`
  - Безопасный retry: `text:` задачи не ретраятся как URL-парсинг.
- `lib/stores/admin-store.ts`
  - Добавлены `TextProcessingOptions` для контроля тяжёлых этапов.
- `components/admin/ArticlesManager.tsx`
  - Убраны fake views, улучшен fallback миниатюр.
- `components/admin/MobileArticleCard.tsx`
  - Добавлен fallback миниатюры для mobile списка.

### 🧹 Аудит
- Проверены временные артефакты в git: критичных мусорных файлов нет.
- Локальные служебные файлы (`.DS_Store`, build artifacts) не отслеживаются (покрыты `.gitignore`).

### 🚀 Релиз
- Версия обновлена: `8.6.7`.
- Изменения готовы для production.
