 ...
 * 
 * Использование:
 *   npm run clean-problematic -- --dry-run  # Только показать что будет удалено
 *   npm run clean-problematic -- --confirm  # Реальное удаление
 */
 ...
 */
function containsRussian(text: string | null | undefined): boolean {
 ...
 */
function isTestArticle(title: string, slugEn?: string, slugPl?: string): boolean {
 ...
 */
function hasExtractionErrors(content: string | null | undefined): boolean {
 ...
 */
function isTooShort(contentEn?: string, contentPl?: string): boolean {
 ...
 */
async function findProblematicArticles(): Promise<ProblematicArticle[]> {
  ...
  const problematic: ProblematicArticle[] = [];
  ...
  for (const article of articles) {
    ...
    if (isTooShort(contentEn, contentPl)) {
      reasons.push(`📏 Слишком короткая (${Math.max((contentEn?.length || 0), (contentPl?.length || 0))} символов)`);
    }
 ...
 */
async function deleteFromSupabase(articleId: number): Promise<boolean> {
  try {
 ...
 */
async function deleteFromWordPress(slugEn?: string, slugPl?: string): Promise<boolean> {
  const slugs = [slugEn, slugPl].filter(Boolean) as string[];
...
async function cleanProblematicArticles() {
  ...
  try {
    ...
    const byReason: Record<string, ProblematicArticle[]> = {};
    ...
    problematic.forEach((article, index) => {
      console.log(`${index + 1}. ID: ${article.id} | "${article.title.substring(0, 60)}${article.title.length > 60 ? '...' : ''}"`);
    ...
    for (const article of problematic) {
      console.log(`🗑️  Удаляем: "${article.title.substring(0, 50)}..." (ID: ${article.id})`);
...
export { cleanProblematicArticles, findProblematicArticles };
