/**
 * 🧹 ОЧИСТКА ПРОБЛЕМНЫХ СТАТЕЙ ИЗ WORDPRESS
 * 
 * Находит и удаляет русские, тестовые и проблемные статьи из WordPress
 */
...
if (!WP_USERNAME || !WP_APP_PASSWORD) {
  ...
  console.error('Set WP_USERNAME/WORDPRESS_USERNAME and WP_APP_PASSWORD/WORDPRESS_APP_PASSWORD environment variables');
  console.error('\nПример:');
...
/**
 * Получает все статьи из WordPress
 */
async function getAllWordPressArticles() {
  ...
  while (hasMore) {
    try {
      const url = `${WORDPRESS_API_URL}/wp-json/wp/v2/posts?per_page=${perPage}&page=${page}&status=any&_fields=id,title,slug,content,excerpt,date,status`;
...
function findProblematicArticles(articles) {
  ...
  for (const article of articles) {
    ...
    const combinedText = `${title} ${content} ${excerpt}`;
    ...
    if (textContent.length > 0 && textContent.length < SHORT_CONTENT_THRESHOLD) {
      reasons.push(`📏 Слишком короткая (${textContent.length} символов)`);
    }

    // Проверка 7: Подозрительный slug
...
/**
 * Удаляет статью из WordPress
 */
async function deleteWordPressArticle(articleId) {
  try {
    const deleteUrl = `${WORDPRESS_API_URL}/wp-json/wp/v2/posts/${articleId}?force=true`;
 ...
 */
async function cleanWordPressArticles() {
  const isDryRun = process.argv.includes('--dry-run');
  const isConfirm = process.argv.includes('--confirm');

  console.log('🧹 ОЧИСТКА ПРОБЛЕМНЫХ СТАТЕЙ ИЗ WORDPRESS\n');
  ...
  try {
    ...
    if (isDryRun || !isConfirm) {
      ...
      console.log('   node scripts/clean-wordpress-articles.js --confirm\n');
      return;
    }
    ...
    for (const article of problematic) {
      ...
      try {
        const success = await deleteWordPressArticle(article.id);
        if (success) {
          deleted++;
          console.log(`  ✅ Удалено`);
        } else {
...
module.exports = { cleanWordPressArticles, findProblematicArticles };
