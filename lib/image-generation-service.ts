...
function generateImagePrompt(params: ImageGenerationParams): string {
  ...
  prompt += ' No text, no watermarks, no logos. Wide landscape format suitable for article header.';
...
export async function generateArticleImage(
  ...
  try {
    ...
    const response = await openai.images.generate({
      ...
      prompt: prompt,
      n: 1,
      size: params.size || '1792x1024', // Landscape format для article headers
...
export async function getUnsplashImage(
  ...
  try {
    ...
    if (!unsplashAccessKey) {
      // Fallback к search-based URL если нет API ключа
    ...
    const url = `${data.urls.raw}&w=1200&h=400&fit=crop`;
