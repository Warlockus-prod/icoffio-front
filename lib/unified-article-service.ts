...
import { advancedUrlParser } from './advanced-url-parser';
...
class UnifiedArticleService {
  ...
  private async extractContentFromUrl(url: string): Promise<any> {
    ...
    try {
      // Используем продвинутый парсер URL
      const extractedContent = await advancedUrlParser.extractContent(url);
  ...
  async checkServicesHealth(): Promise<{
    ...
    return {
      ...
      urlParser: advancedUrlParser.isAvailable()
    };
  }
}
